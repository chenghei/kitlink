  ...
  <div id="map">
    <div class="mapheader boxShadow">
      <div class="mapSelect">
        ...
        <span :class="mapActive === 'normal' ? 'selected':''" @click="mapSelect('normal')">所有设备</span>
      </div>
    ...
    <div id="allmap1" class="boxShadow">
      <!-- leaflet map -->
...
export default {
  ...
  methods: {
      ...
      item.data.forEach(e => {
        if(e.latitude && e.longitude){
          ...
          let data = {
            ...
            'lm_icon': L.icon({ iconUrl: icon, iconSize: [62, 78], iconAnchor: [32, 78], popupAnchor: [0, 0] }),
            infoIcon: e.iconAddr ? e.iconAddr : 'unknown.png',
            devEui: e.devEui,
            name: e.name,
            time: e.updateTime,
      ...
      if (this.mapActive === 'alarm') {
        if(item.latitude && item.longitude){
            ...
            const data = {
              ...
              'lm_icon': L.icon({ iconUrl: icon, iconSize: [62, 78], iconAnchor: [32, 78], popupAnchor: [0, 0] }),
              infoIcon: item.iconAddr ? item.iconAddr : 'unknown.png',
              devEui: item.devEui,
              name: item.name,
    ...
    handleAlarmList(list) {
      ...
      let alarmMark = []
      let alarmType = groupByArray(list,'typeName')
      alarmType.forEach(e => {
        e.data.forEach(k => {
