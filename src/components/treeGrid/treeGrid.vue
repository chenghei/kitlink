<style lang="less" scoped src='./treeGrid.less'></style>
<template>
    <div :style="{'width':'100%','height':tableHeight+'px','overflow-y':'auto','border-bottom':'1px solid #ddd'}" class='autoTbale'>
            ...
            <tbody>
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style='tdStyle(column)'>
                        ...
                        <div v-if="column.type === 'action'" class="treeTable">
                            ...
                            <Tooltip v-for='(action,i) in (column.actions)' :key='i' :content="action.tip" placement="top" :style="{'marginRight':action.marginRight+'px','marginLeft':action.marginLeft+'px'}">
                                <img @click="RowClick(item,$event,index,action.title)"  :src="action.src" class="img1">
                            </Tooltip>
                            <!-- <Tooltip content="删除" placement="top" style="margin-right:40px">
                                <img src="../../assets/images/node/right2.svg" >
                            </Tooltip>
                            <Tooltip content="添加于菜单" placement="top">
                                <img @click="RowClick(item,$event,index,'添加于菜单')"  src="../../assets/images/addMechanism.svg" >
                            </Tooltip> -->
                        </div>
    ...
    methods: {
      ...
      iconRow() {
        ...
        renderBody(row, column, index) {
            return column.key == 'hideInMenu' ? (row[column.key] ? '隐藏' : '显示') : row[column.key]
...
#hl-tree-table th:nth-child(1)>label{
    margin-left:22px
}
