...
<template>
    <div class="content-body">
        <div class="content boxShadow">
            ...
            <Row type="flex"  class="content_select">
                ...
                <i-col span="12" style="width:200px">
                    <i-select filterable clearable v-model="ammeterList.search['proj.id']">
                ...
                <div class="content_select_input">
                    <p>{{$t('name')}}：</p>
                    <Input type="text"  v-model="ammeterList.search.name" style="width:200px"/>
                </div>
                <div class="select_button" @click="init">{{$t('query')}}</div>
            </Row>
            <Table ref="table" :height="tableHeight" :columns="ammeterList.columns" :data="ammeterList.data"></Table>
            <div class="list_bottom">
                <Page :total="ammeterList.total" :page-size="ammeterList.search.pageSize" @on-change="nextPage" show-elevator show-total></Page>
            </div>
        </div>
            ...
            <div class="add_warpper" v-if="show_map" v-show="show_Map">
                ...
                <div class="add_detail">
                    ...
                    <Row class="add_scroll">
                        <FormItem :label="$t('project')+'：'" prop="proj.id" style="margin-top:20px">
                            <i-col style="width:200px;">
                                <i-select filterable clearable v-model="formValidate.proj.id">
                                    <i-option v-for="item in projects" :value="item.id" :key="item.id">{{item.name}}</i-option>
                                </i-select>
                            </i-col>
                        </FormItem>
                        <FormItem :label="$t('name')+'：'" prop="name">
                            <Input type="text" style="width:200px" v-model="formValidate.name" />
                        </FormItem>
                        <FormItem :label="$t('code1')+'：'" prop="code" >
                            <Input type="text" style="width:200px" v-model="formValidate.code" />
                        </FormItem>
                        <FormItem :label="$t('type')+'：'" prop="type" >
                            <Input type="text" style="width:200px" v-model="formValidate.type" />
                        </FormItem>
                        <FormItem :label="$t('protocol')+'：'" prop="protocol" >
                            <Input type="text" style="width:200px" v-model="formValidate.protocol" />
                        </FormItem>
                        ...
                        <FormItem :label="$t('remark')+'：'" prop="remarks">
                            <Input type="textarea" style="width:400px" v-model="formValidate.remarks" />
                        </FormItem>
                    </Row>
        ...
        <Modal v-model="deleteModel.status">
            ...
            <div slot="footer">
                <Button type="success" :loading="deleteModel.loading" @click="deleteAmmeters">{{$t('confirm')}}</Button>
...
export default {
    ...
    data () {
        return {
            ...
            ruleValidate: {
                'proj.id':[
                    { required: true, message: this.$t('ple_select_project'), trigger: 'change' }
                ],
                name:[
                        ...
                        render: (h, params) => {
                            ...
                            btn.push(
                                h('Tooltip',{
                                    ...
                                    on: {
                                        click: () => {
                                            this.title = this.$t('edit_ammeter')
                                            this.show_bg = true
                                            this.show_map = true
                                            this.show_Map = true
    ...
    methods: {
        ...
        init() {
            this.getAmmeterList(this.ammeterList.search).then(res => {
                const data = res.data.rs.data
                this.ammeterList.data = data.list
                this.ammeterList.total = data.count
            })
        },
        nextPage (page) {
            this.ammeterList.search.pageNo = page
            this.init()
        },
        mapClick (e) {
        ...
        handleSubmit (name) {
            this.$refs[name].validate((valid) => {
                if (valid) {
                    this.spinShow = true
                    this.saveAmmeter(this.$refs[name].model).then(result => {
                        this.spinShow = false
                        this.init()
        ...
        deleteAmmeters() {
            ...
            this.deleteAmmeter({ammeterId: this.deleteModel.formValidate.id}).then(res => {
