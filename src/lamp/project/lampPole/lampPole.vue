...
<template>
    <div class="content-body">
        <div class="content boxShadow">
            ...
            <Row type="flex"  class="content_select">
                ...
                <i-col span="12" style="width:200px">
                    <i-select filterable clearable v-model="lightPoleList.search['proj.id']">
                ...
                <div class="content_select_input">
                    <p>{{$t('name')}}：</p>
                    <Input type="text"  v-model="lightPoleList.search.name" style="width:200px"/>
                </div>
                <div class="content_select_input">
                    <p>{{$t('code1')}}：</p>
                    <Input type="text" v-model="lightPoleList.search.code" style="width:200px"/>
                </div>
                <div class="select_button" @click="init">{{$t('query')}}</div>
            </Row>
            <Table ref="table" :height="tableHeight" :columns="lightPoleList.columns" :data="lightPoleList.data"></Table>
            <div class="list_bottom">
                <Page :total="lightPoleList.total" :page-size="lightPoleList.search.pageSize" @on-change="nextPage" show-elevator show-total></Page>
            </div>
        </div>
        ...
        <Modal v-model="deleteModel.status">
            ...
            <div slot="footer">
                <Button type="success" :loading="deleteModel.loading" @click="deleteLampPole">{{$t('confirm')}}</Button>
...
export default {
            ...
            lightPoleList:{
                columns: [
                    ...
                    },
                    {
                        ...
                        render: (h, params) => {
                            return h('span', params.row.proj.powerFee)
                        }
                    },
                    {
                        title: this.$t('remark'),
    ...
    methods: {
        ...
        init() {
            this.getLightPoleList(this.lightPoleList.search).then(res => {
                const data = res.data.rs.data
                this.lightPoleList.data = data.list
                this.lightPoleList.total = data.count
            })
        },
        nextPage (page) {
            this.lightPoleList.search.pageNo = page
            this.init()
        },
        mapClick (e) {
        ...
        handleSubmit (name) {
            this.$refs[name].validate((valid) => {
                if (valid) {
                    this.spinShow = true
                    this.saveLightPole(this.$refs[name].model).then(result => {
                        this.spinShow = false
                        this.init()
        ...
        deleteLampPole() {
            ...
            this.deleteLightPole({lightPoleId: this.deleteModel.formValidate.id}).then(res => {
