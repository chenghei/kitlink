...
export const setUid = (uid) => {
  Cookies.set(UID_KEY, uid, { expires: config.cookieExpires || 1 })
}
...
export const setHid = (id) => {
  Cookies.set(H_KEY, id, { expires: config.cookieExpires || 1 })
}

export const getHid = () => {
  const h = Cookies.get(H_KEY)
  if (h) return h
  else return false
}

export const setSid = (id) => {
  Cookies.set(S_KEY, id, { expires: config.cookieExpires || 1 })
}

export const getSid = () => {
  const sid = Cookies.get(S_KEY)
  if (sid) return sid
  else return false
}
 ...
 * 处理网格组数据，适用树形控件
 */
export const parseJson = (arr) => {
  arr = arr.slice()
  const notifyList = ['不通知', '短信', '邮件', '微信', '语音']
  function toParse (arr) {
    arr.forEach(item => {
      ...
      item.expand = true
      if (item.notifyMethod) {
        item.notify = ''
        item.notifyMethod.split(',').forEach(v => {
          item.notify += notifyList[v] + ' '
        })
      }
