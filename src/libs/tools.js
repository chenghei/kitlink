...
export const hasOneOf = (targetarr, arr) => {
  if (targetarr.length > 0) return targetarr.some(_ => arr.indexOf(_) > -1)
  else return true
}
 ...
 * @return 过滤后的根节点数组
 */
export const deal = (nodes, predicate) => {
  // 如果已经没有节点了，结束递归
  if (!nodes) {
    return
  }
...
/**
 * 递归勾选，返回原数据
 * @param {target[]} nodes 要勾选的节点
 * @param {form[]} form 源数据
 * @param {opera string} opera 操作 checked / selected
 * @return 勾选后的根节点数组
 */
export const checkedArr = (target, form, opera) => {
  if (target && Array.isArray(target)) {
    const toParse = arr => {
      arr.forEach(v => {
        if (target.includes(v.id)) {
          Vue.set(v, opera, true)
        } else {
          Vue.set(v, opera, false)
        }
        if (v.children) {
          toParse(v.children)
        }
